...
app.controller("SPMS_DPCR", function ($scope, $http, filterFilter) {
        ...
        $http.post('../SPMS_DPCR/MFO_getPerOffice', { _OfficeID: s.currentOffice, _DivisionID: s.currentDivision }).then(function (response) {
            ...
            if (data == undefined) {
                ...
                angular.forEach(s.cmfoesdata, function (mfo, keyfirst) {
                    ...
                    if (s.StandardPrOffc.length == 0) {
                        status = false;
                        s.assignOpcrCmfo.push({ status: status, MFOId: mfo.MFOId, MFO: mfo.MFO, indicator: mfo.indicator, officeId: officeid, divisionId: divisionid, indicatorId: mfo.indicatorId, target: mfo.target, targetId: mfo.targetId, description: mfo.description, categoryId: mfo.categoryId, TargetOffcId: mfo.TargetOffcId });
                    }
                    else {
                        angular.forEach(s.StandardPrOffc, function (standard, key) {
                            if (keepGoing2) {
                        ...
                        if (mfo.officeId != null & mfo.divisionId == null | mfo.officeId == null & mfo.divisionId == null) {
                            s.assignOpcrCmfo.push({ status: status, MFOId: mfo.MFOId, MFO: mfo.MFO, indicator: mfo.indicator, officeId: officeid, divisionId: divisionid, indicatorId: mfo.indicatorId, target: mfo.target, targetId: targetid, description: mfo.description, categoryId: mfo.categoryId, TargetOffcId: mfo.TargetOffcId });
    ...
    s.addMFO = function (_newMFO, _catMFO, _newSI) {
        console.log("s.addStandard", s.addStandard)

        if (_newMFO.MFO != null && _newSI.indicator != null && _newSI.target != null && _catMFO.categoryId != null) {
            $http.post('../SPMS_DPCR/MFO_add', { newMFO: _newMFO, catMFO: _catMFO, newSI: _newSI, newStandard: s.addStandard, _OfficeID: s.currentOffice, _DivisionID: s.currentDivision, isCheck: s.isCheck }).then(function (response) {
